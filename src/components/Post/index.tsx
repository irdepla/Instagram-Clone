...
const Post = ({ post }: CardProps) => {


    // const [liked, setLiked] = useState(post.id); // Assuming backend sends `isLiked`
    // const [likesCount, setLikesCount] = useState(post.likesCount); // Assuming likes count is provided

    async function likePost(){
        try {
            const res = await apiClient.patch(`/posts/${post.id}/like`)
            console.log("bosildi");
            console.log("my response is", res);
            console.log("like length is", res.data.likes.length);
        } catch (error: any) {
            console.log(error);
        }
    }
    ...
    return (
        <div className="border w-[468px] h-[585px] m-auto border-gray-300 shadow-md rounded-lg p-4">
            <div className="border border-solid items-center">
            ...
            />
            <div className="like flex justify-between items-center mt-1">
                <div>
                <button onClick={() => likePost()}  className="p-2">
            ...
            <p className="mt-2 font-medium flex justify-between gap-1">
